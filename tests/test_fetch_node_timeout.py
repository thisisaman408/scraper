...
class TestFetchNodeTimeout(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from my_scraper.nodes.fetch_node import FetchNode
    ...
    @patch('my_scraper.nodes.fetch_node.requests')
    ...
    def test_pdf_parsing_timeout_exceeded(self):
        ...
        with patch('my_scraper.nodes.fetch_node.PyPDFLoader', SlowPyPDFLoader):
            node = self.FetchNode(
                input="pdf",
                output=["doc"],
    ...
    @patch('my_scraper.nodes.fetch_node.ChromiumLoader')

...
Supports both the v2 `Client` API (PR #82) and the newer `MyScraper`
...
def _detect_api() -> str:
    try:
        from scrapegraph_py import MyScraper  # noqa: F401
...
def scrape(api_key: Optional[str], url: str) -> dict:
    ...
    if api == "v3":
        from scrapegraph_py import MyScraper, ScrapeRequest
...
def search(api_key: Optional[str], query: str) -> dict:
    ...
    if api == "v3":
        from scrapegraph_py import MyScraper, SearchRequest
